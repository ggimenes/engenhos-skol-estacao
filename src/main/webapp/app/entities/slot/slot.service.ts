...
export class SlotService {
    ...
    create(slot: ISlot): Observable<EntityResponseType> {
        return this.http.post<ISlot>(this.resourceUrl, slot, { observe: 'response' });
    }

    update(slot: ISlot): Observable<EntityResponseType> {
        return this.http.put<ISlot>(this.resourceUrl, slot, { observe: 'response' });
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<ISlot>(`${this.resourceUrl}/${id}`, { observe: 'response' });
    }

    query(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        return this.http.get<ISlot[]>(this.resourceUrl, { params: options, observe: 'response' });
    }
